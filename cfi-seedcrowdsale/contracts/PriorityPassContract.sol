...
contract PriorityPassContract is Owned {
    ...
    //
    // @owner updates data for list of account
    ...
    //
    function updateAccounts(address[] _accountAddresses, uint[] _levels, uint[] _limits) onlyOwner public {
        ...
        for (uint cnt = 0; cnt < _accountAddresses.length; cnt++) {
            require(accounts[_accountAddresses[cnt]].active);
